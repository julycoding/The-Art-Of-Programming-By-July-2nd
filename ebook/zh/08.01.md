...
了解了二叉查找树的基本性质还不够，因为若要理解红黑树的插入和删除，得先理解二叉查找树的插入和删除。磨刀不误砍柴工，咱们再来分别了解下二叉查找树的插入和删除。
...
为了继续保持红黑树的性质，我们可以通过对结点进行重新着色，以及树进行相关的旋转操作，即修改树种某些结点的颜色及指针结构，来达到对红黑树进行插入、删除结点等操作时，继续保持它的性质或平衡。  
...
上文，我们了解了二叉查找树的删除，咱们便来具体了解红黑树的插入操作。红黑树的插入相当于在二叉查找树插入的基础上，为了重新恢复平衡，继续做了插入修复操作。
...
下面，咱们继续了解上述插入代码最后一行调用的RB-INSERT-FIXUP（T,z）具体代码，如下所示：  
...
但当出现以下3种情况后，需要做一些调整，至于如何调整便是按照上面的RB-INSERT-FIXUP（T,z）的代码来进行。
...
>「回顾：经过上面情况1、情况2、情况3等3种插入修复情况的操作示意图，读者自会发现，后面的情况2、情况3都是针对情况3插入>节点4以后，进行的一系列插入修复情况操作，不过，指向当前节点N指针一直在变化。所以，你可以想当然的认为：整个下来，情况>1、2、3就是一个完整的插入修复情况的操作流程」
...
删除修复情况4：当前节点是黑加黑且兄弟是黑色且兄弟节点的两个子节点全为黑色。  
