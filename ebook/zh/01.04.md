...
## 题目描述
输入一个由数字组成的字符串，把该字符串转换成整数并输出。例如：输入字符串"345"，输出整数345。
给定函数原型`int StrToInt(const char *str)` ，完成函数StrToInt，实现字符串转换成整数的功能，不得用库函数atoi（即便准许使用，其对于溢出情况的处理也达不到题目的要求，详情请参看下文）。

## 分析与解法
我们来一步一步分析（共9小节，重点在下文第8小节及后续内容），直至写出第一份准确的代码：

以"345"作为例子：
1. 当扫描到字符串的第一个字符'3'时，由于我们知道这是第一位，所以得到数字3。  
2. 当扫描到第二个字符'4'时，由于前面有一个3，所以前面的3相当于30。得到数字：3\*10+4=34。  
3. 当扫最后的描到字符'5'时，由于前面的34相当于340，所以最终得到的数是：34\*10+5=345。  
因此，此题的思路便是：每扫描到一个字符，我们便把在之前得到的数字乘以10，然后再加上当前字符表示的数字。  

有一些细节需要注意，
1. 正负符号：整数不仅包含数字，还有可能以'+'或者'-'开头以表示正负整数，因此如果第一个字符是'-'号，则要把得到的整数转换成负整数。
...
如图，输入字符串和分析的结果。（假定运行环境是32位系统，且编译环境是VS2008以上）：  
...
```
尝试运行上述程序，会发现：输入某些字符串时，程序出错（下图红叉叉对应条目）:  

![](../images/30~31/30.2.jpg)

开始修复：

1.非法字符问题。如“1a”，正确的结果应该得到1：  
...
2.整型溢出问题。如“24147483648”将产生溢出，正确的结果应该得到0： 
