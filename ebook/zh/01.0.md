...
```java
/* JAVA语言 */
void leftShiftOne(char[] charArray, int n) {
    char t = charArray[0];
    for (int i = 1; i < n; i++) {
...
```java
/* JAVA语言 */
String leftShift(String s, int n, int m) {
    char[] charArray = s.toCharArray();
    while (m-- > 0) {
        leftShiftOne(charArray, n);
    }
...
def rotate(s, n):
    # Python这章打酱油了.
    # 后面Python代码都是这样，我就不一个一个写了。
    # 从索引n拆成左右两半，然后调换位置拼接成新字符串.
