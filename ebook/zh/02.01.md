...
首先进行快速，然后遍历序列中前k个元素。总的时间复杂度：`O（n * log n)+O(k)=O（n * log n）`。

### 解法二

题目没有要求最小的k个数有序，也没要求最后n-k个数有序。既然如此，没有必要对所有元素进行排序。
  
1、遍历n个数，先把最先遍历到的k个数存入大小为k的数组中  
2、找到这k个元素中的最大值kmax。（需要遍历这k个数，时间复杂度为`O（k）`）  
3、遍历剩余n-k个数。假设每一次遍历时元素的值为x。把x与kmax比较：如果`x < kmax`，用x替换kmax，回到第二步；如果`x >= kmax`，则继续遍历。  
...
1、用容量为k的最大堆存储最先遍历到的k个数。
2、遍历剩余n-k个数。假设每一次遍历时元素的值为x。把x与堆顶元素比较：如果`x<kmax`，用x替换kmax，然后更新堆；否则不更新堆。

时间复杂度:`O（k+（n-k）*logk）=O（n*logk）`。使用堆进行查找和更新的时间复杂度均：`O(logk)`（如使用解法二：在数组中找出最大元素，时间复杂度：`O（k））`。
