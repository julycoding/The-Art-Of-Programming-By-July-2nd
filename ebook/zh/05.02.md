...
关于dp[i][j-1]+1, s.t. s[i]在T[0…j-1]中的说明：
 - 其中，若s[i]落在了T[1…j-1]的某个位置，不妨认为是k，因为最小编辑步数的定义，那么，在k+1到j-1的字符，必然是通过插入新字符完成的。因为共插入了(j-k)个字符，故编辑次数为(j-k)次。而字符串S[0…i]经过编辑，得到了T[0…k]，编辑次数为dp[i][j-k]。
- 故：dp[i][j]=dp[i][j-k]+(j-k)
 - 由于最后的(j-k)次是插入操作，可以讲(j-k)逐次规约到dp[i][j-k]中。即：dp[i][j-k]+(j-k)=dp[i][j-k+1]+(j-k-1)
规约到插入操作为1次，
- 得到dp[i][j-k]+(j-k) =dp[i][j-k+1]+(j-k-1)= dp[i][j-k+2]+(j-k-2)=…=dp[i][j-1]+1
