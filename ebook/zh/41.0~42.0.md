...
表面上看，Strassen算法仅仅比通用矩阵相乘算法好一点，因为通用矩阵相乘算法时间复杂度是![equation](http://latex.codecogs.com/gif.latex?{n^3=n^{\log_2^8}})，而Strassen算法复杂度只是![equation](http://latex.codecogs.com/gif.latex?{O(n^{\log _2^7})=O(n^{2.807})。但随着n的变大，比如当n >> 100时，Strassen算法是比通用矩阵相乘算法变得更有效率。
