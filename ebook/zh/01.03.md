...
```java
    ...
    static class WildCardMatcher {
        ...
        static List<String> matches(String input, String pattern) {
            ...
            return matches(input.toCharArray(), pa);
        }

        static List<String> matches(char[] input, String[] pattern) {
            int n = input.length, m = pattern.length;
            List<String> result = new ArrayList<String>();
            for (int i = 0; i < n; ) {
                int left = -1, right = -1;
                for (int j = 0; j < m; j++) {
                    long range = lookBehind(input, i, pattern[j], j != 0);
                    if (range < 0) {   // 在i以后找不到pattern[j]
                        i++;
                        break;
                    } else {
                        i = (int) range + 1;
                        if (j == 0)     // 第一个关键字找到，记左边界，高32位
                            left = (int) (range >> 32);
                        if (j == m - 1)     // 最后一个关键字找到，记右边界
                            right = (int) range;
                    }
                }
                if (left >= 0 && right >= 0)
                    result.add(new String(input, left, right - left + 1));
            }
            return result;
        }

        static long lookBehind(char[] in, int i, String pa, boolean greedy) {
            int len = in.length, pLen = pa.length();
            long left = 0;
            if (len - i > pLen - 1) {
                for (; i < len; i++) {
                    for (int off = i, j = 0; i < len && j < pLen; i++, j++) {
                        char p = pa.charAt(j);
                        if (in[i] == p || p == '?') {
                            if (j == 0)     // 匹配开始点
                                left = i;
                            if (j == pLen - 1) {    // 匹配结束点
                                long ii = -1;
                                if (greedy)     // 贪婪模式下尽可能的再次尝试
                                    ii = lookBehind(in, i + 1, pa, true);
                                return (i > ii ? i : ii) | (left << 32);
                            }
                        } else {
                            i = off;    // 匹配失败，重置为off+1位进行回溯
                            break;
                        }
                    }
                }
    ...
    public static void main(String[] args) {
        System.out.println(WildCardMatcher.matches("assbsavb", "a*b"));
        System.out.println(WildCardMatcher.matches("assbsavb", "a??b"));
        System.out.println(WildCardMatcher.matches("assbsavb", "a?b"));
        System.out.println(WildCardMatcher.matches("assbsavbz", "a??"));
        System.out.println(WildCardMatcher.matches("assbsavbz", "?"));
        System.out.println(WildCardMatcher.matches("assbsavbz", "??sb"));
        System.out.println(WildCardMatcher.matches("assbsavbz", "?s"));
        System.out.println(WildCardMatcher.matches("assbsavbz", "s?"));
        System.out.println(WildCardMatcher.matches("assbsavbz", "s?s"));
        System.out.println(WildCardMatcher.matches("assbsavbz", "s"));
        System.out.println(WildCardMatcher.matches("assbsavbz", "z"));
        System.out.println(WildCardMatcher.matches("assbsavbz", "Z"));
        System.out.println(WildCardMatcher.matches("assbsavbz", "s?b?a"));
        System.out.println(WildCardMatcher.matches("assbsavbcsb", "a*sb"));
        System.out.println(WildCardMatcher.matches("assbsavbcsb", "a*s*b"));
        System.out.println(WildCardMatcher.matches("assbsavbcsb", "a*s?"));
        System.out.println(WildCardMatcher.matches("assbsavbcsb", "a*a?b"));
    }
