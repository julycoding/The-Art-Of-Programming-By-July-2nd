...
不难看出，字符串长度为n，若向右移动k， 则每个元素的位置可以根据计算算出，即i -> (i + k) % n。[完美洗牌](https://github.com/julycoding/The-Art-Of-Programming-By-July/blob/master/ebook/zh/02.09.md)中引入了
圈的概念，将这个圈引入到字符串旋转中。比如0,1,2,3,4,5, 右旋转2，则变成4,5,0,1,2,3,即
...
{
    m %= n;               //若要左移动大于n位，那么和%n 是等价的
    int moved; //移动次数，移动次数达到n结束
    int start; // 圈的开始位置
    char displaced; 
    for (start = 0, moved = 0; moved != n; ++start)
    {
	    displaced = s[start];
	    int i = start;
	    // 处理每个圈
