...
## 分析与解法
### 解法一、暴力移位法
初看此题,最先想到的方法可能就是一位一位移动，我们可以实现一个函数`left_shift_one(char *s, int n)` ，以完成左移动一位的功能
...
## 类似问题
1、链表翻转。给出一个链表和一个数k，比如链表1→2→3→4→5→6，k=2，则翻转后2→1→6→5→4→3，若k=3,翻转后3→2→1→6→5→4，若k=4，翻转后4→3→2→1→5→6，用程序实现。
