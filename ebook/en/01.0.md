...
## Problem Description
  ...
  The operation of ***left rotating*** for a n-character string: Move the first $$$m$$$ characters to the end of the string. For example, we can get `cdefab` by left rotating string `abcdef` with m = 2. Please write  a function to implement the `left rotating` operation. For a n-character string, the time complexity of your function should be O(n) and space complexity should be O(1).



## Idea1: Brute force shifting

When first seeing the problem, it is intuitive for us to think about moving the first $$$m$$$ characters one by one. Based on this idea, we have the function leftshiftone(char *s,int n) to shift one character to the left for the input string ( In C language ).
...
```
Thus, it is easy to move $$$n$$$ characters as follows ( In C language ) \:
...
##Idea2: Rotating with two points
Let's see an example first:

```
Suppose we have a string S "abcdefghi". If we want to move "abc" to the end of S, the process can be as follows:

"abc defghi" -> "def abcghi" _> "def ghiabc"

```

In this case, we can move the first $$$m$$$ characters as a group. Each time we move the group to the left for $$$m$$$ positions. To this end, we can define two points, $$$p1$$$ pointing to ch[0], which denotes the first character of S, and $$$p2$$$ referring to ch[m] which denotes the $$$m-th$$$ character in S.

Each time, we make a $$$m-round$$$ loop. During the loop, we exchange the characters pointed by $$$p1$$$ and $$$p2$$$, and then increase $$$p1$$$ and $$$p2$$$ by 1.

In our example, we can get everything done in two steps:

 - Step 1,exchange "abc" 和 "def", so "abc defghi"->"def abcghi"
 - Step 2,exchange "abc" 和 "ghi", so "def abcghi"->"defghi abc"

So the whole process is like moving "abc" continuously and we get the following strings step by step:
...
We can also see the steps in the following figure:
![](../images/1.1.jpg?raw=true)

In the previous example, we have 9 characters in S. When $$$m = 3$$$, $$$p2$$$ is just pointing to the last character of S finally. So everything is fine. However, what happens when we have more than 9 characters, e.g., 10?

If S is "abcdefghij", we will try it again. Left rotate S with $$$m = 3$$$ which means we should get "defghij abc" finally based on idea2. The process is as follows:

Initial string S: `abcdefghij`

- 1.def abc ghij
- 2.def ghi abc j // **Next, we move "j" one position to the left each time!**
- 3.def ghi ab jc
- 4.defghi a jbc
- 5.def ghi j abc

Another figure is for this case↓:

![](../images/1.2.jpg?raw=true)


ok. Let's summarize idea2 in 4 steps:
